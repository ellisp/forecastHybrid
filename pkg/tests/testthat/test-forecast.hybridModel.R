...
if(require(forecast) & require(testthat)){
  ...
  test_that("Testing invalid inputs", {
    ...
    expect_error(forecast(object = hModel, h = 5, xreg = matrix(1:5, nrow = 5, ncol = 2)))
    expect_error(forecast(object = hModel, h = 5, xreg = matrix("a", nrow = 5, ncol = 2)))
    expect_error(forecast(object = hModel, h = 5, xreg = 1:12))
  })
