...
if(require(fpp) & require(forecast) & require(testthat)){
  ...
  test_that("Testing forecasts with xreg", {
    aa <- hybridModel(wineind, models = "a", xreg = matrix(runif(length(wineind)), nrow = length(wineind)))
    expect_that(forecast(aa, xreg = matrix(rnorm(20), nrow = 20)), not(throws_error()))
