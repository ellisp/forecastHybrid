test_that("the forecast(thetam(...)) returns the same results as thetaf(...)", {
  ...
  expect_identical(sum(fc2$mean - fc1$mean), 0)
  expect_identical(sum(fc2$upper - fc1$upper), 0)
  expect_identical(sum(fc2$lower - fc1$lower), 0)

  # simulated seasonal data
  x <- ts(rnorm(100) + c(0, 1, 2, 3), frequency = 4)
  expect_identical(sum(forecast(thetam(x), h = 4)$mean - thetaf(x, h = 4)$mean), 0)
  ...
  expect_identical(sum(fan1$upper - fan2$upper), 0)
  expect_identical(sum(fan1$lower - fan2$lower), 0)
  ...
  expect_identical(sum(fc3$mean - fc1$mean), 0)
...
test_that("hybrid models with theta give same results as when done manually", {
  ...
  expect_identical(sum((fc2a$mean + fc2b$mean) / 2 - fc1$mean), 0)
})
