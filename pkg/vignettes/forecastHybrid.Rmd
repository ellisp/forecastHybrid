...
## Cross Validation
It can be useful to perform cross validation on a timeseries to model to estimate a model's out-of-sample forecasting performance. The `cvts()` function allows us to do this. For example, let's perform cross validation for a `stlm` model and a `nnetar` model on the `woolyrnq` timeseries. The main important arguments that commonly need adjusting are `rolling` (if `TRUE`, the model will always be fit on a fixed `windowSize` instead of growing by one new observation for each new model fit), `windowSize` (starting length of time series to fit a model), and `maxHorizon` (the forecast horizon for predictions from each model)
